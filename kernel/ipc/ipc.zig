//! IPC: synchronous calls and asynchronous notifications (FR-1.3).
//!
//! The rule: EVERY operation is checked against a capability.
//! No token for the endpoint means no send, no receive and no reply.
//! Tokens carried inside a message are passed by delegation (`derive`),
//! so the receiver can never gain more rights than the sender had, and the
//! transfer lands in the audit log.
...
pub const Message = struct {
    ...
    sender_tid: u32 = 0,
    /// Non-zero = a synchronous call waiting for its reply.
    reply_to: u32 = 0,
...
pub fn Ipc(comptime max_endpoints: usize, comptime queue_depth: usize, comptime max_waiters: usize) type {
    return struct {
        ...
        const Endpoint = struct {
            ...
            head: usize = 0,
            len: usize = 0,
            /// The receiver thread blocked waiting for a message.
        ...
        /// Delegate the tokens attached to a message: each one is re-created
        /// for the receiver as a derived token.
        fn transferCaps(
            registry: anytype,
            msg: *Message,
            from_pid: cap.ProcId,
        ...
        /// Send a message to an endpoint. Requires the send right on it.
        /// In synchronous mode the sending thread blocks until the reply.
        pub fn send(
            self: *Self,
            registry: anytype,
            scheduler: anytype,
            ...
            if (mode == .synchronous) {
                ...
                slot.filled = false;
                // Keep the request: the reply takes the caller pid from it.
                slot.msg = msg;
                scheduler.block(sender_tid) catch {};
            }
        }

        /// Receive. Requires the recv right. On an empty queue the thread
        /// blocks and returns here after waking (getting null this time).
        pub fn recv(
            self: *Self,
            registry: anytype,
            scheduler: anytype,
        ...
        /// Reply to a synchronous call: unblocks the calling thread.
        pub fn reply(
            self: *Self,
            registry: anytype,
            scheduler: anytype,
            ...
            // Tokens in a reply are delegated too, never copied.
        ...
        /// Take the delivered reply (called by the woken thread).
...
const Fixture = struct {
    ...
    fn setup(self: *Fixture) !void {
        ...
        self.server_cap = try self.reg.issueRoot(server_pid, obj, .{
            ...
            .send = true,
            .grant = true,
        }, .any, .{ .purpose = "endpoint owner" }, 0);
        self.client_cap = try self.reg.derive(self.server_cap, server_pid, client_pid, .{ .send = true }, .any, .{ .purpose = "service client" }, 0);
...
test "ipc: asynchronous send and receive" {
    var f = Fixture{};
    try f.setup();

    try f.ipc.send(&f.reg, &f.sched, Fixture.client_pid, f.client_tid, f.client_cap, f.ep, Message.withBytes(7, "hello"), .asynchronous, 0);
...
test "ipc: a synchronous call blocks the client until the reply" {
    var f = Fixture{};
    try f.setup();
...
test "ipc: an empty queue blocks the receiver, a send wakes it" {
    var f = Fixture{};
    try f.setup();
...
test "ipc: a full queue applies backpressure" {
    var f = Fixture{};
    try f.setup();
    for (0..4) |_| {
...
test "ipc: passing a token in a message delegates, it does not copy" {
    var f = Fixture{};
    try f.setup();

    // The client holds a file token and hands it to the server over IPC.
    ...
    var msg = Message.withBytes(3, "here is a file");
    msg.caps[0] = file_cap;
    msg.cap_count = 1;
    ...
    // Revoking the original also kills the transferred one.
...
test "ipc: a token cannot be forwarded without the grant right" {
    var f = Fixture{};
    try f.setup();
