//! An 8x8 bitmap font for ASCII 32..126.
//!
//! One byte per row, the most significant bit is the leftmost pixel.
//! The kernel owns its glyphs: after ExitBootServices there is no firmware
//! console left, so text on the framebuffer has to come from somewhere.
...
pub fn glyph(c: u8) *const [glyph_height]u8 {
    if (c < first_char or c > last_char) return &data[('?' - first_char)];
    return &data[c - first_char];
}

const data = [_][glyph_height]u8{
...
comptime {
    if (data.len != last_char - first_char + 1) @compileError("font table size mismatch");
}
